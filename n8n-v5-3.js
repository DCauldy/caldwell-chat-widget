...
(function() {
  ...
  function handleButtonClick(action) {
    ...
    userMessageDiv.textContent = action === "scheduleCall" ? "I'd like to schedule a call!" : "Action Triggered!";
...
async function startNewConversation() {
  ...
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 1000)); // Small delay for realism
    ...
    // Smart parsing if the bot sends a [button:Button Text] style instruction
    if (responseText.includes("[button:")) {
      const buttonLabel = responseText.match(/\[button:(.*?)\]/)[1];
      const button = document.createElement('button');
      button.textContent = buttonLabel;
